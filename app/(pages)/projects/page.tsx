...
export default function ProjectsPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const readyProjects = data.filter(
          (project: Project) =>
            project.status === "ready" && project.availability
        );
        setProjects(readyProjects);
        setFilteredProjects(readyProjects);
      }
    } catch (error) {
  ...
  const handleFilter = (filters: any) => {
    ...
    if (filters.category && filters.category !== "all") {
      filtered = filtered.filter(
        (project: Project) => project.category === filters.category
      );
    }
    ...
    if (filters.complexity && filters.complexity !== "all") {
      filtered = filtered.filter(
        (project: Project) => project.customizable === filters.complexity
      );
    }
